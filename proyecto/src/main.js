...
onclick="mostrarAleatorio()">Aleatorio</button>

<button class="c-nav-item"
onclick="mostrarFavoritos()">Favoritos</button>
...
const res = await
fetch(`https://pokeapi.co/api/v2/pokemon?limit=${totalPokes}`);
...
const tipos = ["All", "normal", "fighting", "flying", "poison", "ground",
...
filtro.innerHTML = tipos.map(tipo => `<button
data-tipo="${tipo}">${tipo}</button>`).join("");
...
/other/official-artwork/${id}.png" width="auto" height="60" loading="lazy"
alt="${name}">
<p>${name}</p>
...
listaFiltrada = lista.filter(pokemon => pokemon.url &&
pokemon.url.includes("/" + texto));
...
document.getElementById("app").innerHTML = `<p>Error al cargar Pokémon
de tipo "${untipo}".</p>`;
}
}
}
...
favoritos.push({ id, name: nombre, url:
...
<img
src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon
/other/official-artwork/${data.id}.png" alt="${data.name}" height="120"
width="auto">
<p>${data.name}</p>
<p>#${data.id}</p>
<p>${tipoPoke}</p>
...
const contenedor = document.createElement("section");
contenedor.classList.add("c-lista");
contenedor.innerHTML = generarLista(nuevosAleatorios); // Usamos la función
reutilizable
app.appendChild(contenedor);
}
...
misPokes += `
<div class="c-unpoke c-mios-pokemon poke-${i}"
onclick="mostrarDetalle('${i}')">

<img

src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon
/${i}.png" width="auto" height="45" loading="lazy" alt="${i}">
